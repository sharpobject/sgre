...
local path = string.sub(..., 1, string.len(...) - string.len(".skins.SG.skin"))
...
local largefont = love.graphics.newFont(12)
local imagebuttonfont = love.graphics.newFont(15)
local bordercolor = {160, 160, 160, 255}
...
skin.controls.button_text_nonclickable_color        = {255, 255, 255, 100}
...
-- multichoice
skin.controls.multichoice_body_color                = {255, 230, 166, 255}
skin.controls.multichoice_text_color                = {155, 94, 33, 255}
skin.controls.multichoice_text_font                 = smallfont
skin.controls.multichoice_border_color              = {207, 152, 43, 255}

-- multichoicelist
skin.controls.multichoicelist_body_color            = {255, 230, 166, 255}

-- multichoicerow
skin.controls.multichoicerow_body_nohover_color     = {255, 230, 166, 255}
skin.controls.multichoicerow_body_hover_color       = {248, 183, 65, 255}
skin.controls.multichoicerow_text_nohover_color     = {157, 96, 35, 255}
skin.controls.multichoicerow_text_hover_color       = {157, 96, 35, 255}
...
-- textinput
skin.controls.textinput_border_color                = {231, 165, 114, 255}
skin.controls.textinput_body_color                  = {250, 250, 250, 200}
skin.controls.textinput_indicator_color             = {0, 0, 0, 255}
skin.controls.textinput_text_normal_color           = {128, 82, 36, 255}
...
skin.controls.textinput_highlight_bar_color         = {248, 183, 65, 255}
...
function skin.OutlinedRoundRectangle(x, y, width, height, ovt, ovb, ovl, ovr)
	...
	-- bottom
	if not ovb then
		love.graphics.rectangle("fill", x+1, y + height - 1, width-2, 1)
	end
	
	-- left
	if not ovl then
		love.graphics.rectangle("fill", x, y+1, 1, height-2)
	end
	
	-- right
	if not ovr then
		love.graphics.rectangle("fill", x + width - 1, y+1, 1, height-2)
	end
...
function skin.DrawFrame(object)
	...
	local topbarimage = skin.images["sgframe-topbar.png"]
	local topbarimage_width = topbarimage:getWidth()
	local topbarimage_height = topbarimage:getHeight()
	local topbarimage_scalex = (width-2)/topbarimage_width
	local topbarimage_scaley = 24/topbarimage_height
	local bordercolor = skin.controls.frame_border_color
	local separatorcolor = skin.controls.frame_separator_color
	...
	love.graphics.draw(topbarimage, x+1, y+1, 0, topbarimage_scalex, topbarimage_scaley)
	...
	if icon then
		...
		love.graphics.print(name, x + iconwidth + 10, y + 7)
	else
		love.graphics.setColor(namecolor)
		love.graphics.print(name, x + 5, y + 7)
	end
	
	-- frame border
	love.graphics.setColor(bordercolor)
	skin.OutlinedRoundRectangle(x, y, width, height)
	
	love.graphics.setColor(separatorcolor)
	love.graphics.rectangle("fill", x+1, y+25, width-2, 1)
end
...
function skin.DrawButton(object)
	...
	local bordercolor = skin.controls.button_border_color
	...
	local image_hover = skin.images["sgbutton-hover.png"]
	local scaley = (height-2)/image_hover:getHeight()
	local insetcolor = skin.controls.button_inset_color
	local insetwidth = skin.controls.button_inset_width
	
	if not enabled or not clickable then
		-- button body
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(skin.images["sgbutton-unclickable.png"], x+1, y+1, 0, width-2, scaley)
		-- button text
		love.graphics.setFont(font)
	...
	if object.toggleable then
		if hover then
			if down then
				-- button body
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(skin.images["sgbutton-down.png"], x+1, y+1, 0, width-2, scaley)
				-- button text
				love.graphics.setFont(font)
			...
			if object.toggle then
				...
				-- button body
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(skin.images["sgbutton-nohover.png"], x+1, y+1, 0, width-2, scaley)
				-- button text
				love.graphics.setFont(font)
	...
	skin.OutlinedRectangle(x + 1, y + 1, width - 2, height - 2)
	love.graphics.setColor(bordercolor)
	love.graphics.points(x + 1.5, y + 1.5)
	love.graphics.points(x + 1.5, y + height - 1.5)
	love.graphics.points(x + width - 1.5, y + 1.5)
	love.graphics.points(x + width - 1.5, y + height - 1.5)

	-- the thin, dark line just below the top
	love.graphics.setColor(insetcolor)
	love.graphics.rectangle("fill", x+3, y+2, width-6, insetwidth)
...
function skin.DrawProgressBar(object)
	...
	love.graphics.rectangle("fill", x, y+1, barwidth, height-2)
	love.graphics.setFont(font)
...
function skin.DrawScrollArea(object)
	...
	if bartype == "vertical" then
		love.graphics.rectangle("fill", x+1, y, width-2, height)
	elseif bartype == "horizontal" then
		love.graphics.rectangle("fill", x, y+1, width, height-2)
	end
...
function skin.DrawScrollBar(object)
	...
	if dragging then
		local image = skin.images["sgbutton-down.png"]
		local imageheight = image:getHeight()
		local scaley = (height-2)/imageheight
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(image, x+1, y+1, 0, width-2, scaley)
		love.graphics.setColor(bordercolor)
		skin.OutlinedRoundRectangle(x, y, width, height)
	elseif hover then
		local image = skin.images["sgbutton-hover.png"]
		local imageheight = image:getHeight()
		local scaley = (height-2)/imageheight
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(image, x+1, y+1, 0, width-2, scaley)
		...
		local image = skin.images["sgbutton-nohover.png"]
		local imageheight = image:getHeight()
		local scaley = (height-2)/imageheight
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(image, x+1, y+1, 0, width-2, scaley)
...
function skin.DrawOverList(object)
	...
	local width = object:GetWidth()
	local height = object:GetHeight()
	local bordercolor = skin.controls.list_border_color
...
function skin.DrawTabButton(object)
	...
	if tabnumber == ptabnumber then
		-- button body
		local gradient = skin.images["sgbutton-hover.png"]
		...
		-- button body
		local gradient = skin.images["sgbutton-nohover.png"]
		local gradientheight = gradient:getHeight()
		local scaley = height/gradientheight
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(gradient, x, y, 0, width, scaley)
...
function skin.DrawMultiChoice(object)
	...
	object:SetHeight(20) -- hack: new default height
	local skin = object:GetSkin()
	local x = object:GetX()
	local y = object:GetY()
	local width = object:GetWidth()
	...
	love.graphics.setColor(255, 255, 255, 150)
	love.graphics.draw(image, x + width - 16, y + 3)
...
function skin.DrawSliderButton(object)
	...
	if not enabled then
		local image = skin.images["sgbutton-unclickable.png"]
		local imageheight = image:getHeight()
		local scaley = (height-2)/imageheight
		-- button body
		love.graphics.setColor(255, 255, 255, 255)
